...
observation = degrads @ card
observation = np.random.rand(*observation.shape).astype(np.float32) / 20 + observation


class SingleSolver(nn.Module):

    def __init__(self) -> None:
        super(SingleSolver, self).__init__()

        self.cards = nn.Parameter(torch.rand(*card.shape))
        self.degrad = torch.from_numpy(degrads).cuda()

    def forward(self):
        cards = torch.clamp(self.cards, 1e-5, 1)
        b = self.degrad @ cards
        return b, cards
...
def main():
    ...
    solver_net = SingleSolver().cuda()
    optimizer = torch.optim.Adam(solver_net.parameters(), 0.001)
    observation = torch.from_numpy(observation).cuda() #  / observation.max()
    ...
    for iteration in pbar:
        ...
        if iteration % 500 == 0:
            pbar.set_description(
                ...
                + f' fideloss = {fideloss.detach().item()}'
                + f' tvloss = {tvloss.detach().item()}'
            )

    cards = cards.detach().cpu().numpy()
    for i in range(cards.shape[1]):
        print('='*6, i)
        gt = card[:, i]
        pred = cards[:, i]
        # pred *= gt.mean() / pred.mean()

        plt.cla()
        plt.plot(gt)
        plt.plot(pred)
        # plt.show()
        plt.savefig(f"log/simu/res_{i}.png")
