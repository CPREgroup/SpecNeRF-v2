...
def OctreeRender_trilinear_fast(rays, tensorf, chunk=args.chunk_size, N_samples=-1, ndc_ray=False, white_bg=True, \
    ...
    rgbs, alphas, depth_maps, weights, uncertainties, dist_losses, spec_maps, rgb_resduals, rgb_preds = [], [], [], [], [], [], [], [], []
    ...
    for chunk_idx in range(N_rays_all // chunk + int(N_rays_all % chunk > 0)):
        ...
        rgb_map, depth_map, dist_loss, spec_map, phi, rgb_resdual, rgb_pred = tensorf(rays_chunk, poseids_chunk, filters_chunk, is_train=is_train, white_bg=white_bg, \
        ...
        rgb_resduals.append(rgb_resdual)
        rgb_preds.append(rgb_pred)
    
    return torch.cat(rgbs), None, torch.cat(depth_maps), None, None, torch.cat(dist_losses).mean(), torch.cat(spec_maps), phi, torch.cat(rgb_resduals), torch.cat(rgb_preds)
...
def evaluation(test_dataset:LLFFDataset,tensorf, args, renderer, savePath=None, N_vis=5, prtx='', N_samples=-1,
    ...
    for idx, samples in tqdm(enumerate(test_dataset.all_rays[0::img_eval_interval]), file=sys.stdout):
        ...
        rgb_map, _, depth_map, _, _, _, spec_map, _, _, _ = \
